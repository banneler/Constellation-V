...
export function initializeAbmSequenceEditor(config) {
    ...
    // Reset local state for the editor
...
async function handleAddStep() {
    if (state.isEditingDetails || state.editingStepId) {
    ...
    const modalBody = `
        ...
        <input type="number" id="modal-step-number" value="${nextNum}" required>
        <label>Type</label>
        <input type="text" id="modal-step-type" required placeholder="e.g., Email, Call, LinkedIn">
        <label>Subject (for Email)</label>
        <input type="text" id="modal-step-subject" placeholder="Optional">
        <label>Message (for Email/Notes)</label>
        <textarea id="modal-step-message" placeholder="Optional"></textarea>
        <label>Delay (Days after previous step)</label>
        <input type="number" id="modal-step-delay" value="0" required>
        <label>Assign To</label>
        <select id="modal-step-assigned-to">
...
function renderSteps() {
    ...
    stepsTableBody.innerHTML = state.steps.map(step => {
        const isEditing = state.editingStepId === step.id;
        return `
            <tr data-id="${step.id}">
                <td>${step.step_number}</td>
                <td>
                    ${isEditing ? `<input type="text" class="form-control edit-step-type" value="${step.type || ''}">` : step.type || ''}
                </td>
                <td>
                ...
                <td>
                    ...
                    </select>` : step.assigned_to || 'Sales'}
                </td>
                <td>
                     ${isEditing ? `<input type="number" class="form-control edit-step-delay" value="${step.delay_days || 0}" min="0">` : step.delay_days || 0}
                </td>
                <td>
...
function render() {
    ...
    state.containerElement.innerHTML = `
        <h3>ABM Sequence Details</h3>
        <hr>
        <div class="form-grid">
            <div class="full-span-grid-item">
                <label for="sequence-name-input">Name:</label>
                <input type="text" id="sequence-name-input" class="form-control" value="${state.sequence.name || ''}" ${state.isEditingDetails ? '' : 'disabled'}>
            </div>
            <div class="full-span-grid-item">
                <label for="sequence-description-textarea">Description:</label>
                <textarea id="sequence-description-textarea" class="form-control" ${state.isEditingDetails ? '' : 'disabled'}>${state.sequence.description || ''}</textarea>
            </div>
        </div>
        ...
        <div class="action-buttons">
            <button id="add-step-btn" class="btn-secondary ${state.isEditingDetails || state.editingStepId ? 'hidden' : ''}">Add Step</button>
        </div>
    `;

    renderSteps();

    // Attach event listeners for this specific render
    state.containerElement.querySelector('#edit-sequence-details-btn')?.addEventListener('click', () => {
        state.isEditingDetails = true;
        render();
    });
    state.containerElement.querySelector('#cancel-edit-sequence-btn')?.addEventListener('click', () => {
        state.isEditingDetails = false;
        render();
    });
