...
        showClosedLost: false, // NEW: State for the toggle
        currentUserQuota: 0,
        allUsersQuotas: [],
...
    const showClosedLostToggle = document.getElementById('show-closed-lost-toggle'); // NEW: Selector for the toggle
...
    }
    
    // NEW: Helper function to get deals based on the toggle state
    function getFilteredFunnelDeals() {
        const futureDeals = getFutureDeals();
        
        // Filter deals based on the state
        return futureDeals.filter(deal => {
            if (deal.stage === 'Closed Won') return false; // Always hide Closed Won from funnel
            if (deal.stage === 'Closed Lost' && !state.showClosedLost) return false; // Conditionally hide Closed Lost
            return true;
        });
    }
...
        const dealsToRender = getFilteredFunnelDeals();

        const dealsWithAccount = dealsToRender.map((deal) => ({ ...deal, account_name: state.accounts.find((a) => a.id === deal.account_id)?.name || "N/A" }));
...
        // MODIFIED: Filter out stages we don't want to see
        const stages = state.dealStages.map(s => s.stage_name)
            .filter(stage => {
                if (stage === 'Closed Won') return false;
                if (stage === 'Closed Lost' && !state.showClosedLost) return false;
                return true;
            });
...
        });
        
        // MODIFIED: Use the new helper function for the funnel total
        const totalFunnel = getFilteredFunnelDeals().reduce((sum, deal) => sum + (deal.mrc || 0), 0);
        
...
        }
        if (viewAllDealsBtn) {
This           viewAllDealsBtn.addEventListener('click', async () => {
                const isManager = state.currentUser.user_metadata?.is_manager === true;
...
        // NEW: Event listener for the Show Closed Lost toggle
        if (showClosedLostToggle) {
            showClosedLostToggle.addEventListener('change', (e) => {
                state.showClosedLost = e.target.checked;
                localStorage.setItem('deals_show_closed_lost', state.showClosedLost);
                render(); // Re-render the current view (list or board)
                renderDealsMetrics(); // Also update metrics
          _ });
        }
    }
    
